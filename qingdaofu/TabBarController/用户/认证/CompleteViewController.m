...
- (void)viewDidLoad {
    ...
    self.rightItem.style = UIBarButtonSystemItemEdit;
...
{
    if (!_cell0) {
        _cell0 = [[AuthenBaseView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 44)];
        _cell0.label.text = @"| 认证信息";
        _cell0.label.textColor = kBlueColor;
        _cell0.textField.userInteractionEnabled = NO;
        _cell0.textField.placeholder = @"";
...
{
    if (!_cell1) {
        _cell1 = [[CompletView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 200)];
    }
    return _cell1;
}
